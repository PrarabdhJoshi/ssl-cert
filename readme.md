# CloudFlare Programming Exercise
## Prarabdh Joshi

HTTP-based RESTful API for managing Customers and their Certificates.

> # Customer
    - Has a name
    - Has an email address
    - Has a password
    - May have zero to many Certificates
    
> # Certificate
    - Belongs to one and only one Customer
    - Can be either active or inactive 
    - Has a private key
    - Has a certificate body

## Features

- Creating/Deleting Customers
- Viewing all Customers
- Viewing Customer by Id
- Creating Certificates
- Listing all of user's active certificates
- Activating/Deactivating Certificates
- Notify an external system when activation/deactivation occurs
- Data persistence through postgresSQL

## Tech

- Node JS
- Koa
- Typescript
- Postgres
- Openssl
- Docker
- password-hash

## Installation

Pre-requisites: [Node.js](https://nodejs.org/) v12+, docker >=20.10.0, npm >= 6.14.8

Install the dependencies and devDependencies and start the server.

```sh
cd user-cert
npm install
npm run run-app
```

## API Documentation

> #### creating new customer
    - POST: ${baseUrl}/customer
    - body: {
            name: string //required
            password: string //required
            email: string // required
        }
    - response: {
        status: [201, 403, 500]
        message/error: string
    }
##### Example (201): 
> Request: 
    curl --header "Content-Type: application/json" -d "{\"name\":\"test-user\", \"email\": \"testing@test.com\", \"password\":\"pass\"}" http://localhost:9008/customer 
    Response:
    {"status":201,"message":"user successfully created with id: 7193f400-7f01-4d40-b30d-f67207860d33"}
    
##### Example (403): 
> Request: 
    curl --header "Content-Type: application/json" -d "{}" http://localhost:9008/customer 
    Response:
    {"status":403,"error":"malformed request. Please provide all name, password and email in the body"}

> #### View customer by id
    - GET: ${baseUrl}/customer/:id
    - response: {
        status: [200, 404, 500]
        data: object
    }
##### Example (200): 
> Request: 
    curl http://localhost:9008/customer/7193f400-7f01-4d40-b30d-f67207860d33
    Response:
    {status:200, data:[{"id":"7193f400-7f01-4d40-b30d-f67207860d33", name:"test-user", email: "testing@test.com"}]}

##### Example (404): 
> Request: 
    curl http://localhost:9008/customer/non-existing
    Response:
    {"status":404,"error":"customer with id: non-existing does not exist"}
    
> #### View existing Customers
    - GET: ${baseUrl}/customer
    - response: {
        status: [200, 500]
        data: object | error: string
    }
##### Example: 
> Request: 
    curl http://localhost:9008/customer
    Response:
    {"status":200,"data":[{"id":"7193f400-7f01-4d40-b30d-f67207860d33","name":"test-user","email":"testing@test.com","password":"sha1$2d9ffcce$1$5c4e3e1633447ba0d049dbedcb30f6f73b00218c"}]}
    
> #### Delete existing Customer
    - DELETE: ${baseUrl}/customer/:id
    - response: {
        status: [204, 404, 500]
        data: null | error: string
    }
##### Example (204): 
> Request: 
    curl -X DELETE http://localhost:9008/customer/7193f400-7f01-4d40-b30d-f67207860d33
    Response:
    {"status":204,"data":null}

##### Example (404): 
> Request: 
    curl -X DELETE http://localhost:9008/customer/non-existent
    Response:
    {"status":404,"error":"customer with id: non-existent does not exist"}


> #### Creating new Certificate
    - POST: ${baseUrl}/certificate
    - body: {
            customerId: string //required
            active: boolean //required
            domainName: string // required
        }
    - response: {
        status: [201, 403, 404, 500]
        message/error: string
    }
##### Example (201): 
> Request: 
    curl --header "Content-Type: application/json" -d "{\"customerId\":\"40802f0d-d030-40f4-8572-a2e18372d161\", \"active\": false, \"domainName\":\"www.test.com\"}" http://localhost:9008/certificate 
    Response:
    {"status":201,"message":"certificate successfully created with id: fcf4ec54-cfdb-477b-ba73-03c29af6320f"}  

##### Example (404): 
> Request: 
    curl --header "Content-Type: application/json" -d "{\"customerId\":\"non-existent\", \"active\": false, \"domainName\":\"www.test.com\"}" http://localhost:9008/certificate
    Response:
    {"status":404,"error":"customer with id: undefined does not exist"}

##### Example (403): 
> Request: 
    curl --header "Content-Type: application/json" -d "{\"customerId\":\"non-existent\", \"active\": false}" http://localhost:9008/certificate
    Response:
    {"status":403,"error":"malformed request. Please provide customer id, active and domainName field"}
    
> #### View Customer's active certificates
    - GET: ${baseUrl}/customer/:id/active-certificates
    - response: {
        status: [200, 404, 500]
        data: object | error: string
    }

##### Example (200): 
> Request: 
    curl hhttp://localhost:9008/customer/40802f0d-d030-40f4-8572-a2e18372d161/active-certificates
    Response:
    {"status":200,"data":[{"id":"38b67343-920e-4c3d-8b11-098b65451b69","private_key":"-----BEGIN PRIVATE KEY-----\\nhmI5WiFTRutk/aR/5ExTen7S\n-----END PRIVATE KEY-----\n","cert_body":"-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n","customer_id":"40802f0d-d030-40f4-8572-a2e18372d161","active":true}]}
    
##### Example (404, no active Certificate(s)): 
> Request: 
    curl http://localhost:9008/customer/5ea2c6f2-f737-4730-8b0d-ff971f2690f8/active-certificates
    Response:
    {"status":404,"error":"no active certificates exist for the customer."}

##### Example (404, non-existent customer): 
> Request: 
    curl http://localhost:9008/customer/non-existent/active-certificates
    Response:
    {"status":404,"error":"customer with id: 5ea2c6f2-f737-4730-8b0d-ff971f2690f does not exist"}
    
> #### Activate/DeActivate Certificate
    - PATCH: ${baseUrl}/certificate/:certificateId
    - body: { active: boolean }
    - response: {
        status: [204, 403, 404, 500]
        data: object | error: string
    }
    
##### Example (204): 
> Request: 
    curl -X PATCH -H "Content-Type: application/json" \
    -d '{"active": false}' \
     http://localhost:9008/certificate/fcf4ec54-cfdb-477b-ba73-03c29af6320f/
    Response:
    {"status":204,"data":null}
    
##### Example (404, non-existent Certificate): 
> Request: 
    curl -X PATCH -H "Content-Type: application/json" \
    -d '{"active": false}' \
     http://localhost:9008/certificate/non-existent/
    Response:
    {"status":404,"error":"certificate with id: non-existent does not exist"}

##### Example (403, malformed request): 
> Request: 
    curl -X PATCH -H "Content-Type: application/json" \
    -d '{}' \
     http://localhost:9008/certificate/non-existent/
    Response:
    {"status":403,"error":"malformed request. Please provide active field in the body"}
    
##### Example (sending activation notification): 

    
    




